...
def stock():
    ...
    if request.method == 'GET':
        return render_template('11.stock.html', menu=menu, weather=get_weather_main())
    else:
        df = pd.read_csv('result.csv',sep=',',  index_col=0)
        code = request.form['code']
        code = df[df['종목명'] == code ]['종목코드'].to_list()[0]
        name = f'{request.form["code"]}({code})'
        ...
        return render_template('11.stock_res.html', menu=menu, weather=get_weather_main(), mtime=mtime, name = name)
