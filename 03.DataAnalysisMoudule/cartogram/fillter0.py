    ...
    for shop in all_shop:
        ...
        if df['시도명'][0] in ['서울특별시', '광주광역시', '대구광역시', '대전광역시', '부산광역시', '울산광역시', '인천광역시']:
            ...
            for city in df['ID'].unique():
                tmp = df[df['ID'] == city]
                bur = len(tmp[tmp['상호명'].str.contains('버거킹')])
                mac = len(tmp[tmp['상호명'].str.contains('맥도날드')]) 
                kfc = len(tmp[tmp['상호명'].str.contains('kfc') | tmp['상호명'].str.contains('케이에프씨') | tmp['상호명'].str.contains('케이에프시') ]) 
                lot = len(tmp[tmp['상호명'].str.contains('롯데리아')])
                mom = len(tmp[tmp['상호명'].str.contains('맘스터치')])
