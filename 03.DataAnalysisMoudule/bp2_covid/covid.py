    ...
    if request.method == 'GET': 
       ...
       '충북', '충남', '전북', '전남', '경북', '경남', '제주', '검역']
        ...
        if request.form['date'] == 'day':
            ...
            s_mon = '2020-' + request.form['s_mon'][:2] + '-01'
            e_mon = '2020-' + request.form['e_mon'][:2] + '-31'
            rows = t_chart(data,s_mon,e_mon)
            df1 = pd.DataFrame(rows)
            df1['확진월'] = df1[1].apply(lambda r: r.split('-')[1] + '월' )
        ...
        return render_template('covid-19/makechart_res.html', menu=menu, weather=get_weather(), label=label, val = val, area= area, count = count, chart_list=chart_list, sido_data=request.form['sido_data'])
    ...
    if request.method == 'GET': 
       ...
       '양천구', '영등포구', '광진구', '동대문구', '도봉구', '용산구', '강북구', '중구', '기타']
        ...
        if request.form['date'] == 'day':
            ...
            pick = '확진월'
            s_date = '2020-' + request.form['s_mon'][:2] + '-01'
            e_date = '2020-' + request.form['e_mon'][:2] + '-31'
